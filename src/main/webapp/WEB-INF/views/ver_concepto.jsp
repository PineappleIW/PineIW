...
Preview.callback.autoReset = true;  // make sure it can run more than once</script>
 ...
 <!-- -->
	<div class="titulo"> <h2>${concep.nombre}</h2></div>
	
	
  <div class="row">
    <div class="divbusqueda">
    
    <div id="marked-mathjax-preview2">${concep.descripcion}</div>
    ...
    <div class="col-md-3">
      <ul class="nav nav-pills nav-stacked">
        ...
        <td class="success"> <button type="submit" value="${c.nombre}" name="conce">${c.nombre}</button>
        </c:forEach>
      </ul>
      </div>
   </form>
	...
	<c:when test="${concep.tipo eq 'examen'}">
		<form>   
      	...
      	<ul class="nav nav-pills nav-stacked">
        		...
        		<td class="success"> <button type="submit" value="${s.nombre}" name="sol">solución de ${s.user.nombre}   puntuacion:${s.puntuacion}</button>
        		</c:forEach>
      		</ul>
    	</div>
    	</form>
